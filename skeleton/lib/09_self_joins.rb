...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
  SELECT
    stops.name, r1.company, r1.num
  FROM
    routes AS r1
    JOIN stops ON stops.id = r1.stop_id
  WHERE
    r1.num IN (
      SELECT
        num
      FROM
        routes AS r2
        JOIN stops ON r2.stop_id = stops.id
      WHERE
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT
    r1.num, r1.company, stops2.name, r2.num, r2.company
  FROM
    routes AS r1
    JOIN stops AS stops1 ON r1.stop_id = stops1.id
